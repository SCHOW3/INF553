...
def create_user_movie(lists_of_list):
    ...
    for l in lists_of_list:
        for elem in range(1, len(l)):
            to_return[int(l[0][1:])].append(int(l[elem]))
    return to_return
...
def create_movie_user(lists_of_list):
    ...
    for l in lists_of_list:
        for elem in range(1, len(l)):
            to_return[int(l[elem])].append(int(l[0][1:]))
    return to_return
...
def find_recommendations(jaccard_similarities, user_to_movie_dict):
    ...
    for user in user_list:
        ...
        for similar_user in similar_user_list:
            ...
            count_to_movies_keys = sorted(count_to_movie_keys, True)
...
if __name__ == "__main__":
    if len(sys.argv) != 3:
        print("Usage: Steven_Chow_lshrec.py input.txt output.txt", file=sys.stderr)
        sys.exit(-1)
    ...
    user_to_movie_lines = spark.read.text(sys.argv[1]).rdd.map(lambda r: r[0]).map(lambda x: x.split(','))\
    .collect()
    ...
    write_to_output_file(recommendation_list, sys.argv[2])
